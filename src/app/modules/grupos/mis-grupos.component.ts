...
@Component({
  ...
  template: `
    <section class="space-y-6">
      <div class="space-y-1">
        <h2 class="text-xl font-bold text-ip-primary">Mis grupos</h2>
        <p class="text-sm text-ip-muted">Grupos organizacionales a los que perteneces</p>
      </div>
      ...
      <div *ngIf="loading" class="flex items-center justify-center py-16">
        <div class="flex flex-col items-center gap-3">
          ...
          <p class="text-sm text-ip-muted">Cargando grupos...</p>
        </div>
      </div>
      ...
      <div *ngIf="!loading && groups.length > 0" class="space-y-4">
        <article *ngFor="let g of groups"
          class="rounded-2xl bg-ip-surface border border-ip-border p-5 hover:border-ip-border transition-colors">
          <div class="flex flex-wrap items-center gap-2 mb-3">
            <span [class]="roleBadgeClass(g.role === 'ADMIN')">
              {{ g.role === 'ADMIN' ? 'Administrador' : 'Miembro' }}
            </span>
            <span class="text-[11px] text-ip-primary0 uppercase tracking-wide">{{ g.groupType }}</span>
          </div>
 
          <h3 class="text-lg font-semibold text-ip-primary mb-1">{{ g.groupName }}</h3>
          <p class="text-sm text-ip-muted mb-1">{{ g.hierarchyPath.displayPath }}</p>
          <p *ngIf="g.hierarchyPath.rootName" class="text-xs text-ip-primary0 mb-4">
            {{ g.hierarchyPath.rootName }}
          </p>
 
          <div class="flex flex-wrap gap-4 text-sm text-ip-secondary mb-4">
            <span class="flex items-center gap-1.5">
              <svg class="w-4 h-4 text-ip-primary0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="rounded-2xl border border-dashed border-ip-border bg-ip-surface/50 p-5 space-y-3">
          <h4 class="text-sm font-semibold text-ip-primary">¿Quieres unirte o crear otra organización?</h4>
          <div class="flex flex-wrap gap-3">
            ...
            <span class="text-ip-secondary">·</span>
            <button type="button"
...
export class MisGruposComponent implements OnInit {
  ...
  roleBadgeClass(isAdmin: boolean): string {
    return isAdmin
      ...
      : 'text-[11px] font-semibold px-2.5 py-1 rounded-md bg-ip-surface/50 text-ip-secondary';
  }
}
